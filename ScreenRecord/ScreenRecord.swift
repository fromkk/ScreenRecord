...
public protocol ScreenRecorderDelegate: class {
    ...
    func screenRecordDidCompletion(screenRecord: ScreenRecord) -> Void
...
public protocol ScreenRecordable: class {
    ...
    func stop(completion: Completion?) -> Void
}
...
extension ScreenRecord: ScreenRecordable {
    ...
    public func stop(completed: Completion? = nil) {
        if !self.isRecording {
            return
        }
        ...
        dispatch_async(self.queue) { 
            ...
            writer.finishWritingWithCompletionHandler({ [weak self] in
                dispatch_async(dispatch_get_main_queue(), {
                    completed?(url: self?.currentURL)
                    if let strongSelf = self {
                        strongSelf.delegate?.screenRecordDidCompletion(strongSelf)
                    }
                })
            })
        }
